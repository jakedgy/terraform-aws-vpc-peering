...
locals {
  acceptor_cidr_block_associations  = length(var.acceptor_cidr_block_associations) > 0 ? var.acceptor_cidr_block_associations : toset(data.aws_vpc.acceptor.0.cidr_block_associations.cidr_block)
  requestor_cidr_block_associations = length(var.requestor_cidr_block_associations) > 0 ? var.requestor_cidr_block_associations : toset(data.aws_vpc.requestor.0.cidr_block_associations.cidr_block)
}
