...
resource "aws_route" "requestor" {
  count                     = module.this.enabled && length(var.acceptor_cidr_block_associations) == 0 ? length(distinct(sort(data.aws_route_tables.requestor.0.ids))) * length(data.aws_vpc.acceptor.0.cidr_block_associations) : 0
